classdef ParkingLotnoPQ < handle
   properties
       Lot
       targetCar
       Graph
   ...
   methods
       function obj = ParkingLotnoPQ(lot, heuristic, debug)
           if nargin<3
               obj.debug = false;
               if nargin <2
                   obj.heuristic = true;
       ...
       function solutions = depart(obj, targetCar)
           ...
           while ~isempty([obj.queue1,obj.queue2,obj.queue3,obj.queue4])
               ...
               addQ = 0;        %the queue which the node will be added to
               while ~isempty(actionSet)
                   addQ = addQ + 1;
                   if obj.heuristic
                       action = obj.bestAction(focusCar, stateLot, actionSet);
