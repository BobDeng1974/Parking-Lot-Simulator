classdef ParkingLot < handle
   ...
   methods
       ...
       function solutions = depart(obj, targetCar)
           ...
           while ~isempty([obj.queue1,obj.queue2,obj.queue3,obj.queue4])
               ...
               if obj.isVert(focusCar,stateLot)
                   actionSet = obj.ActionSet(~ismember(obj.ActionSet,["r","l"]));
               else
                   actionSet = obj.ActionSet(~ismember(obj.ActionSet,["u","d"]));
               end
               ...
               for actionIdx = 1:length(actionSet)
                   ...
                   if obj.stateExists(newLot, newFocusCar)
                      state = obj.state2node(newLot,newFocusCar);
                      if ismember(state,obj.queue1)
                          obj.queue1 = obj.queue1(obj.queue1~=state);
                          obj.addToQueue(newNodeIndex,actionIdx)
                      elseif ismember(state,obj.queue2)
                          obj.queue2 = obj.queue2(obj.queue2~=state);
                          obj.addToQueue(newNodeIndex,actionIdx)
                      elseif ismember(state,obj.queue3)
                          obj.queue3 = obj.queue3(obj.queue3~=state);
                          obj.addToQueue(newNodeIndex,actionIdx)
                      elseif ismember(state,obj.queue4)
                          obj.queue4 = obj.queue4(obj.queue4~=state);
                          obj.addToQueue(newNodeIndex,actionIdx)
                      end
                   end
       ...
       function bestAction = bestAction(obj, car, lot, actionSet)
